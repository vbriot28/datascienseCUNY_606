...
1.  What command would you use to extract just the counts of girls baptized? Try
    it!
vb--- Added command for viewing girls counts ---
...
2.  Is there an apparent trend in the number of girls baptized over the years?  
    ...
    vb--- The number of girls baptized each year seemed to increase with a few exceptions;
          from year 1640 - 1650 which shows a sharp drop
          around year 1657, 
          around year 1666, 
          around year 1693, 
          and finally around year 1705
          It appears that the boys plot shows similar trend.  Ideally, we should plots both on the same graph with 
...
the future.
vb---
```{r plot-proportion of boys-vs-year, eval=TRUE}
plot(x = arbuthnot$year, y = arbuthnot$boys / (arbuthnot$boys + arbuthnot$girls), type = "l")
```
    The ratio of boys/total over time oscilate within range 0.505 and 0.535
    ---
...
Dimension of data set present:
```{r basic information about data set present, eval=TRUE}
dim(present)
```   

Years included in data set:
```{r year-present, eval=TRUE}
present$year
```

Variables for data set:
```{r variable-names, eval=TRUE}
names(present)
```

-   How do these counts compare to Arbuthnot's? Are they on a similar scale?
Not quite, there are about 300 larger in present data set than in the Arbuthnot data set..

```{r means value of both data set, eval=TRUE}
mean(arbuthnot$boys)
...
mean(present$girls)
```
Ratio of present/Arbuthnot for both boys and girls (based on respective mean)
```{r means value ratio, eval=TRUE}
...
```{r plot-boy-to-girl ratio-vs-year, eval=TRUE}
plot(present$year, present$boys/present$girls, type = "l")
```

The ratio boy to girl in the "present" data set is greater than one so the observation made by Arbuthnot that boys are born in greater proportion than girls holds true in the U.S.  However, from the graph it appears that the ratio is diminishing.
...
```{r most U.S. births, eval=TRUE}
max(present$boys + present$girls)
```
   
Maximum birth = 4268326

Determination of year for maximum births count:
```{r year for most births, eval=TRUE}
present_new <- cbind(present, present$boys + present$girls)
present_new <- cbind(present_new, present_new[,4] == max(present_new[,4]))
sum(present_new [,1] * present_new[,5])
