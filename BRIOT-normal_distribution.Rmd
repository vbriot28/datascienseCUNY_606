...
***Male Histogram:***    
```{r histogram-male, eval=TRUE}

hist(mdims$hgt, xlim = c(140, 200), ylim = c(0, 100), breaks = seq(140, 210, 5),
                xlab = "Height (centimeter)", ylab = "Frequency", 
                main = "Heights of Male population", prob = FALSE, col="lightblue")
```
   
From the histogram, the distribution of heights for Male population is unimodal,
fairly symetrical, centered around the value of 178.

***Female Histogram:***
```{r histogram-female, eval=TRUE} 

hist(fdims$hgt, xlim = c(140, 200), ylim = c(0, 100), breaks = seq(140, 210, 5), 
                xlab = "Height (centimeter)", ylab = "Frequency", 
                main = "Heights of Female population", prob = FALSE, col="lightcoral")
```
    
From teh histogram, the distribution of heights for Female population appears unimodal,
slightly skewed to the left, centered around the value of 165.
...
Based on the plot that is overlaid on top of the histogram for the female 
population, we can see that the data follow a nearly normal distribution. 
We may have some area of concerns in the 155-165 range since the plot cuts 
throught the histogram column.  We may want to narrow each column to see 
if the data fits the curve better.

```{r histogram-smaller-bins}    
hist(fdims$hgt, ylim = c(0, 0.08), breaks = seq(140, 190, 2), probability = TRUE)
x <- 140:190
y <- dnorm(x = x, mean = fhgtmean, sd = fhgtsd)
lines(x = x, y = y, col = "blue")
```
With the narrower intervals, it appears that the data fits a nearly normal 
distribution but we may have some irregularity around 158 and 164.
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    data?
```{r sim_norm-plot, eval=TRUE}
qqnorm(sim_norm)
qqline(sim_norm)

```
The plot for the simulated data set "sim_norm" is very close to the one for our actual data.
...
Yes, the normal probability plot for 'fdims$hgt' looks similar to the plots created
for the simulated data.  This would indicate the the female heights data set
has nearly normal distribution.

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.

First we will calculate the mean and standard deviation for the female weights data set
and store the values in appropriate variables.

```{r female-weight, eval = TRUE}
fwgtmean <- mean(fdims$wgt)
fwgtsd   <- sd(fdims$wgt)

```

We will also plot the histogram for this data set. We will determine the minimum and maximum 
for the data to help us determine the best value to use to plot histogram.

```{r female-weight-min-max, eval = TRUE}

fwgtmin <- min(fdims$wgt)
fwgtmax <- max(fdims$wgt)
```
Minimum = `r fwgtmin`
Maximum = `r fwgtmax`

We will now plot the histogram for this data set.

```{r female-weight-histogram, eval= TRUE}
hist(fdims$wgt, xlim = c(40, 110), ylim = c(0, 80), breaks = seq(40, 110, 5), 
                xlab = "Weight (kg)", ylab = "Frequency", 
                main = "Weights of Female population", prob = FALSE, col="lavender")
```

From the histogram, it appears that the data distribution is unimodal, 
fairly sysmetrical although skewed to the right, with peak around 55-60.  
We may have presence of outliers with high value weight (above 100 kg).

we will overlay a normal distribution curve ontop of histogram.

```{r female-weight-normal, eval = TRUE}
hist(fdims$wgt, xlim = c(40, 110), ylim = c(0, 0.1), breaks = seq(40, 110, 5), 
                xlab = "Weight (kg)", ylab = "Frequency %", 
                main = "Weights of Female population", prob = TRUE, col="lavender")
x <- 40:110
y <- dnorm(x = x, mean = fwgtmean, sd = fwgtsd)
lines(x = x, y = y, col = "red")
```

We will now plot the normal probability plot (qq-plot) for the female weights.
```{r qq-fweight, eval=FALSE}
qqnorm(fdims$wgt)
qqline(fdims$wgt)
```

From the normal probability plot for this data set, although the data is very closed
to the line for the "middle" values, it is clear that as we approaches low weights or
high weights, the values are no longer close to the line.  It does appear that 
we have 2 outliers in with weight values greater than 100 kg. 
The normal probability plot show the right skewedness that we had observed with
the histogram, with the upper part curving away (up) from line.

Using a simulated population, we will now plot a qq-plot for the simulated population.
Also using qqnormism we will compare the data set against multiple simulation.

```{r female-weights-sim, eval = TRUE}
sim_norm_fw <- rnorm(n = length(fdims$wgt), mean = fwgtmean, sd = fwgtsd)
...
The qq-plot for our data set (upper-left) differs from the simulations plot
in the higher weight range of greater than 100 kg.
The skewedness of our data distribution and the presence of possibly outliers
is impacting the distribution of our data set.  
We would conclude based on the plots that our data set is not following a normal distribution.
...
*** Questions:***
a. What is the probability that a female height is between 158 cm and 168 cm?
b. What is the probability that a female weight is less than 54 kg?

For question (a) we will determine the area under the normal distribution curve
for x = 168 and then for x = 158, then subtract them.  
We will then calculate same empirically by summing up the observances 
between 158 and 168 and divide by the total number of observances.

```{r pnorm-question-a, eval = TRUE}
z1 <- pnorm(q = 168, mean = fhgtmean, sd = fhgtsd)
z2 <- pnorm(q = 158, mean = fhgtmean, sd = fhgtsd)

z1 - z2

sum(fdims$hgt > 158 & fdims$hgt < 168) / length(fdims$hgt)
```

The results from the 2 different methods of calculating the answers 
for question (a) are very close as expected since we had concluded 
that the height distribution for female was nearly normal.

Let us now consider the answer of the 2nd question.
```{r pnorm-question-b, eval = TRUE}
z3 <- pnorm(q = 54, mean = fwgtmean, sd = fwgtsd)

sum(fdims$wgt < 54) / length(fdims$wgt)
```
There is a significant difference between the 2 calculations.  
This seems to point to the fact the weight distribution for female is not nearly normal.
...
We are considering the variables in these distribution as continuous 
but they are more discreet in nature.
...
```{r qqplot-fkne, eval=TRUE}
qqnorm(fdims$kne.di)
qqline(fdims$kne.di, col="blue")

```

Based on this probability plot, we would surmize that this variable is skewed to
the right, with possible presence of outliers around value of 24.

We will now plot the histogram to confirm have a different look this data set.
Again, to understand the lower and upper limit for x axis, we will compute the
min and max of variable.

```{r hist-fkne, eval=TRUE}
min(fdims$kne.di)
max(fdims$kne.di)

hist(fdims$kne.di, xlim = c(14, 26), ylim = c(0, 0.4), breaks = seq(14, 26, 1), 
                xlab = "Knee Diameter (both knees) (cm)", ylab = "Frequency %", 
                main = "Knee Diameter of female athlete (Sum of both knees)",
                prob = TRUE, col="thistle")

mean_kf <- mean(fdims$kne.di)
sd_kf <- sd(fdims$kne.di)

x <- seq(14, 26, .1)
y <- dnorm(x = x, mean = mean_kf, sd = sd_kf)
lines(x = x, y = y, col = "blue")
```

From the histogram, we can confirm our original observation that this variable 
(Knee diameter (sum of measurement for both kneew) for female athletes) has a
right skweed distribution.
